  ...
  try {
    ...
    python.on("close", async (code) => {
      try {
        ...
        if (errorOutput) console.log("Python error output:", errorOutput);
        ...
        try {
          ...
          console.log("Parsed Python output:", predictions);
        } catch (e) {
          console.error("Failed to parse Python output:", e);
          throw new Error("Failed to parse prediction results");
        }

        // Check if it's not a coffee bean
        if (predictions.error === 'Not a coffee bean') {
          res.json(predictions);  // Pass through the not-coffee-bean response directly
          return;
        }
        ...
        console.log("Sending to frontend:", {
          success: true,
          predictions: { flavor, aroma, body, acidity, overall_quality }
        });
        ...
        console.error("Error in Python process handler:", err);
        fs.unlink(req.file.path, () => {});
        res.status(500).json({ success: false, error: err.message });
      }
    });
  } catch (err) {
    console.error("Error in prediction endpoint:", err);
